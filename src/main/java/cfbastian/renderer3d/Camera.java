...
public class Camera {
    ...
    public void setTheta(double theta) {
        ...
        this.deltaU = new Vector3(uSpacing*Math.sin(theta), -uSpacing*Math.cos(theta), 0D);
        this.deltaV = new Vector3(vSpacing*Math.cos(phi)*Math.cos(theta), vSpacing*Math.cos(phi)*Math.sin(theta), vSpacing*Math.sin(phi));
        this.viewportUpperLeft = VectorMath.add(viewportCenter, VectorMath.scale(VectorMath.normalize(VectorMath.add(VectorMath.scale(deltaU, -1D), VectorMath.scale(deltaV, -1D))), halfViewportDiagonal));
        this.upperLeftPixel = VectorMath.add(viewportUpperLeft, VectorMath.scale(deltaU, 0.5), VectorMath.scale(deltaV, 0.5));
        calculateRays();
    }
...
//
    ...
    public Vector3 getViewportUpperLeft() {
        return VectorMath.add(viewportUpperLeft);//, VectorMath.scale(deltaU, Application.WIDTH), VectorMath.scale(deltaV, Application.HEIGHT));
    }
}
